...
The application build writes to `dist-openrouter/`, including the linked essay at `/perspective/`. The owning repository is `0xTrey/trey-work-website`. Cloudflare Workers Static Assets serves the production build at `https://openrouter.trey.work/` using `applications/openrouter/wrangler.jsonc`. The earlier `openroute.trey.work` spelling remains an alias for existing links. The Worker has no runtime bindings or secrets.
...
The timeline groups related events into eight stages, ending with Trey's current Folloze work. Its 2025 stage uses first local commits in Trey's fasting tracker (January 22) and PNW Snow Tracker (February 8), plus [Anthropic's February 24, 2025 introduction of Claude Code](https://www.anthropic.com/news/claude-3-7-sonnet). Trey's launch-week Claude Code use, Gauntlet AI acceptance in June, early company-brain work with custom GPTs and ChatGPT Projects, and December Clawdbot use are his account and should be confirmed before publication. The early company-brain milestone does not claim that the later formal pilot existed in 2025. The Today row reflects Trey's description of his AI and GTME responsibilities and current enterprise sales activity; naming Zoom, Equinix, and SailPoint does not claim that those projects closed.
